package com.triamatter.epharma.network.web;

public class API {
    public static final String GET_CATEGORY = "http://riverainteriors.com/epharma/category.php";
    public static final String GET_SUBCATEGORY = "http://riverainteriors.com/epharma/subcategory.php";
    public static final String GET_PRODUCT = "http://riverainteriors.com/epharma/product.php";
    public static final String GET_LATEST_PRODUCT = "http://riverainteriors.com/epharma/latest_product.php";
    public static final String GET_SEARCH = "http://riverainteriors.com/epharma/search.php";

    public static final String POST_SIGNUP = "http://192.168.0.104/epharma/signup.php";
    public static final String POST_SIGNIN = "http://192.168.0.104/epharma/signin.php";

    public static final String POST_INSERT_ORDER = "http://192.168.0.104/epharma/order_insert.php";
    public static final String POST_INSERT_SHIPPING = "http://192.168.0.104/epharma/shipping_insert.php";
}
